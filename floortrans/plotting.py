...
def _register_cmap_compat(cmap):
    """Register colormap on both older and newer Matplotlib versions."""
    if hasattr(matplotlib, 'colormaps') and hasattr(matplotlib.colormaps, 'register'):
        try:
            matplotlib.colormaps.register(cmap, force=True)
        except TypeError:
            # Older signatures may not support force.
            try:
