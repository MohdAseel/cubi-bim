...
def evaluate(args, log_dir, writer, logger):
    ...
    with torch.no_grad():
        total_images = len(data_loader) if args.num_images is None else min(args.num_images, len(data_loader))
        for count, val in tqdm(enumerate(data_loader), total=total_images,
